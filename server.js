...
/*
  ...
  We intentionally do NOT use templateId or sendingProfileId here.

  Blackleaf accepted the neutral wording during template validation,
  but the send endpoint reported the saved template and sender IDs
  as nonexistent.

  This uses Blackleaf's direct-send path with a neutral body and
  landingUrl supplied separately.
*/
...
- Live inventory is not connected. Never guess stock.
- Use check_live_inventory for inventory questions.
...
- After tools, answer naturally without mentioning APIs or Blackleaf.
`;
...
const TOOLS=[
...

  {
    ...
    description:
      "Check live inventory connection. Never guess stock.",
    parameters:{
      type:"object",
      properties:{
        query:{
  ...
  if(
    ...
    if(!ctx.callerNumber){
      return{
        success:false,
        message:
          "Caller phone number is unavailable. " +
          "Do not claim a text was sent."
      };
    }
    ...
    try{
      ...
      return{
        ...
        message:
          ...
          "Give the website address instead."
      };
    }
  }
